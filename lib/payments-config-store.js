const { getSettingsState } = require('./settings-store');
...
async function getPaymentsConfig(options = {}) {
    ...
    cache.inflight = (async () => {
        const settingsState = await getSettingsState({ strict: true }).catch(() => ({ ok: false, settings: null }));
        if (!settingsState?.ok || !settingsState?.settings) {
            if (cache.value) return cache.value;
            throw new Error('payments_settings_unavailable');
        }
        const payments = buildPaymentsConfig(settingsState.settings?.payments || {});
