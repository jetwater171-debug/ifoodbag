...
const DEFAULT_REMARKETING_MESSAGE = 'Oi {nome}, seu pagamento ainda esta pendente. Retome seu pedido: {link}';
...
function renderRemarketingMessage(template, { name = '', link = '' } = {}) {
    const cleanLink = String(link || '').trim();
    if (!cleanLink || cleanLink.length > 160) return '';
    const firstName = pickText(name).split(/\s+/)[0] || 'cliente';
...
function prepareRemarketingSms({ lead = {}, smsConfig = {}, baseUrl = '' } = {}) {
    ...
    const message = renderRemarketingMessage(smsConfig?.remarketingMessage, {
        name: offer.customerName || lead?.name,
        link: recoveryUrl
    });
...
function paidResult(status, paid) {
    return { ok: true, paid: paid === true, status: String(status || '').trim().toLowerCase() };
}
