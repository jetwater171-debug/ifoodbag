...
const RECOVERY_EXTRA_DISCOUNT_PERCENT = 30;
...
function normalizeRecoveryDiscount(value) {
    return Number(value) >= RECOVERY_EXTRA_DISCOUNT_PERCENT
        ? RECOVERY_EXTRA_DISCOUNT_PERCENT
...
function resolveRecoveryOffer(lead = {}) {
    ...
    return {
        ...
        rewardId: pickText(reward?.id, payload?.rewardId, payload?.reward_id, pix?.rewardId, 'bag'),
...
function resolveRecoverySessionGrant(lead = {}, sessionIdInput = '', discountPercentInput = RECOVERY_DISCOUNT_PERCENT) {
    const offer = resolveRecoveryOffer(lead);
    const basis = resolveRecoveryGrantBasis(lead, offer);
    const sessionId = pickText(sessionIdInput, offer?.sessionId);
    if (!sessionId || !basis?.txid || Number(basis?.originalAmount || 0) <= 0) return null;
    return {
        sessionId,
        txid: basis.txid,
        originalAmount: basis.originalAmount,
        discountPercent: normalizeRecoveryDiscount(discountPercentInput)
    };
}

function resolveRecoveryOfferForSessionGrant(lead = {}, grant = {}) {
    const offer = resolveRecoveryOffer(lead);
    const basis = resolveRecoveryGrantBasis(lead, offer);
    ...
    const discountPercent = normalizeRecoveryDiscount(grant?.discountPercent);
    return {
        ...offer,
        originalTxid: basis.txid,
        originalAmount: basis.originalAmount,
        discountPercent,
        discountedAmount: roundMoney(basis.originalAmount * (1 - discountPercent / 100)),
        canRecover: Boolean(offer?.txid && !offer?.paid)
    };
}
