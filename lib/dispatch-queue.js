...
async function fetchRemarketingCandidates(delayMinutes, activatedAt) {
    ...
    url.searchParams.set('and', `(updated_at.gte.${activatedAt})`);
...
async function reconcileRemarketingSmsJobs() {
    ...
    const activatedAtMs = Date.parse(String(smsConfig.remarketingActivatedAt || ''));
    if (!Number.isFinite(activatedAtMs)) {
        return { ok: true, skipped: true, reason: 'remarketing_activation_not_recorded', scanned: 0, queued: 0 };
    }
    const delayMinutes = normalizeDelayMinutes(smsConfig.remarketingDelayMinutes);
    const candidates = await fetchRemarketingCandidates(delayMinutes, new Date(activatedAtMs).toISOString());
    ...
    const jobs = candidates.rows.map((lead) => {
        ...
        if (!shouldBackfillRemarketingSms(offer, new Date(activatedAtMs).toISOString(), delayMinutes)) return null;
