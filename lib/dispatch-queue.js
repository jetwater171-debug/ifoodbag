...
async function dispatchRemarketingSms(job = {}) {
    ...
    if (!lead) return { ok: true, skipped: true, reason: 'remarketing_lead_not_found' };

    const settings = await getSettings().catch(() => ({}));
    const prepared = prepareRemarketingSms({
        lead,
        smsConfig: asObject(settings?.smsmais),
    ...
    if (!livePayment?.ok) {
        return { ok: false, reason: livePayment?.reason || 'payment_status_check_failed' };
    }
    if (livePayment.paid) {
        return { ok: true, skipped: true, reason: 'payment_confirmed_before_sms' };
    }

    return dispatchSmsMaisJob('sms', prepared.payload || {});
}
