...
const { verifyAdminPassword, issueAdminCookie, verifyAdminCookie, requireAdmin } = require('./admin-auth');
const { getSettings, getSettingsState, saveSettings, defaultSettings } = require('./settings-store');
const { invalidatePaymentsConfigCache } = require('./payments-config-store');
const {
    buildPaymentsConfig,
    mergePaymentSettings,
    resolveGatewayFromPayload
} = require('./payment-gateway-config');
const { sendUtmfy } = require('./utmfy');
const { updateLeadByPixTxid, getLeadByPixTxid, updateLeadBySessionId, getLeadBySessionId } = require('./lead-store');
const { sendPushcut } = require('./pushcut');
const { sendSmsMaisSms, sendSmsMaisVoice, getSmsMaisBalance } = require('./smsmais');
const { resolveRecoveryOffer, resolveRecoveryGrantBasis } = require('./remarketing-recovery');
const { buildRemarketingSalesReport } = require('./remarketing-sales');
const {
...
const { mergePaymentHistory, normalizePaymentHistoryStatus } = require('./lead-payment-history');
const { enqueueDispatch, enqueueManualRemarketingSms, processDispatchQueue } = require('./dispatch-queue');
const {
    getIpBlacklist,
    addBlockedIp,
    removeBlockedIp,
    findBlockedIp,
    normalizeClientIp
} = require('./ip-blacklist');
const { getOfficialHosts, listCloneEvents } = require('./clone-detector-store');
...
const REMARKETING_SALES_SELECT_FIELDS = [
    'session_id', 'name', 'email', 'cpf', 'phone', 'stage', 'last_event',
    'pix_txid', 'pix_amount', 'shipping_id', 'shipping_name',
    'utm_source', 'utm_campaign', 'utm_term', 'payload', 'updated_at', 'created_at'
].join(',');
...
function sanitizeSettingsForAdmin(settingsData = {}) {
    ...
    payload.payments.gateways.clownpay.subaccountApiKey = maskSecret(payload.payments.gateways.clownpay.subaccountApiKey);
...
function leadStepDisplayLabel(step = '', { charge = false } = {}) {
    if (step === 'remarketing_recovery') return 'Remarketing';
...
function extractGatewaySalesEntries(row = {}) {
    ...
    const pushEntry = (item = {}, source = 'history') => {
        ...
        entries.push({
            ...
            shippingName: String(item?.shippingName || (txid === currentTxid ? row?.shipping_name || payload?.shipping?.name : '') || '').trim(),
            shippingId: String(item?.shippingId || (txid === currentTxid ? row?.shipping_id || payload?.shipping?.id : '') || '').trim(),
    ...
    if (isLeadPaid(row, payload) && currentTxid && !entries.some((item) => item.txid === currentTxid)) {
        pushEntry({
            status: 'paid',
            ...
            shippingId: row?.shipping_id || payload?.shipping?.id || '',
...
async function fetchLeadsPage({ range = null, query = '', limit = 50, offset = 0, select = LEADS_SELECT_FIELDS, enrich = true } = {}) {
    const url = new URL(`${SUPABASE_URL}/rest/v1/leads`);
    ...
    const enrichedRows = enrich
        ? await enrichLeadAttribution(sourceRows).catch(() => sourceRows)
        : sourceRows;
    return {
        ok: true,
...
async function fetchAllLeadsForExport({ range = null, query = '', maxRows = MAX_LEADS_EXPORT_ROWS, pageSize = 1000, select = LEADS_SELECT_FIELDS, enrich = true } = {}) {
    const rows = [];
    let offset = 0;
    let truncated = false;
        ...
        const mergedPaymentsInput = {
            ...
            gateways: {
                ...
                clownpay: {
                    ...
                    subaccountApiKey: pickSecretInput(bodyClownPay.subaccountApiKey, currentClownPayGateway.subaccountApiKey || ''),
        ...
        const payload = {
            ...
            smsmais: hasSmsMaisSection
                ? {
                    ...
                    remarketingActivatedAt: bodySmsMais.remarketingEnabled === true
                        ? (currentSmsMais.remarketingEnabled === true && currentSmsMais.remarketingActivatedAt
                            ? String(currentSmsMais.remarketingActivatedAt)
...
async function createLeadRemarketingLink(req, res, sessionIdParam = '') {
    ...
    const recoveryUrl = `${baseUrl}/remarketing?sessionId=${encodeURIComponent(offer.sessionId)}`;
...
function smsMaisSupabaseHeaders() {
    return {
        apikey: SUPABASE_SERVICE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`,
        'Content-Type': 'application/json'
...
function remarketingLeadForAdmin(lead = {}, delayMinutes = 10) {
    if (leadHasAnyPaidPayment(lead)) return null;
    const offer = resolveRecoveryOffer(lead);
    const payload = asObject(lead?.payload);
    const personal = asObject(payload?.personal);
    const phone = String(lead?.phone || personal?.phone || personal?.phoneDigits || '').replace(/\D/g, '');
    if (!offer?.canRecover || offer.status !== 'pending' || !offer?.txid || !offer?.sessionId || !phone) return null;
    const createdAtMs = Date.parse(String(offer?.createdAt || lead?.updated_at || lead?.created_at || ''));
    ...
    return {
        ...
        txid: offer.txid,
        name: String(offer.customerName || lead?.name || 'Lead sem nome').trim(),
        phone,
        email: String(lead?.email || personal?.email || '').trim(),
        amount: Number(offer.originalAmount || 0),
        discountedAmount: Number(offer.discountedAmount || 0),
        discountPercent: Number(offer.discountPercent || 0),
        offerName: String(offer.offerName || 'Pedido selecionado').trim(),
        rewardId: String(offer.rewardId || '').trim(),
        gateway: String(offer.gateway || '').trim(),
        createdAt: String(offer.createdAt || lead?.updated_at || lead?.created_at || ''),
        waitingMinutes,
        automaticReady: waitingMinutes >= Number(delayMinutes || 10)
    };
}

function isKitRemarketingLead(lead = null) {
    return lead?.rewardId === 'kit_entregador';
}

async function smsMaisRemarketingLeads(req, res) {
    if (req.method !== 'GET') {
        res.status(405).json({ error: 'Method not allowed' });
        return;
    }
    ...
    const settingsData = await getSettings().catch(() => ({}));
    const delayMinutes = clamp(settingsData?.smsmais?.remarketingDelayMinutes || 10, 1, 1440);
    const kitOnly = String(firstQueryValue(req.query?.kit) || '') === '1';
    const limit = clamp(req.query?.limit || (kitOnly ? 200 : 500), 20, kitOnly ? 200 : 1000);
    const query = String(req.query?.q || '').trim().toLowerCase();
    const pageSize = kitOnly ? 250 : limit;
    const maxScanned = kitOnly ? 10000 : limit;
    const leads = [];
    let scanned = 0;
    let exhausted = false;
    while (scanned < maxScanned && leads.length < limit) {
        const take = Math.min(pageSize, maxScanned - scanned);
        const url = new URL(`${SUPABASE_URL}/rest/v1/leads`);
        url.searchParams.set('select', 'session_id,name,email,phone,city,state,cep,shipping_name,shipping_price,pix_txid,pix_amount,last_event,payload,created_at,updated_at');
        url.searchParams.set('pix_txid', 'not.is.null');
        if (kitOnly) {
            url.searchParams.set('or', '(payload->reward->>id.eq.kit_entregador,payload->>rewardId.eq.kit_entregador,payload->>reward_id.eq.kit_entregador,payload->pix->>rewardId.eq.kit_entregador)');
            url.searchParams.set('order', 'payload->>pixCreatedAt.desc.nullslast,updated_at.desc');
        } else {
            url.searchParams.set('order', 'updated_at.desc');
        }
        url.searchParams.set('limit', String(take));
        url.searchParams.set('offset', String(scanned));
        const response = await fetchFn(url.toString(), { headers: smsMaisSupabaseHeaders() }).catch(() => null);
        if (!response?.ok) {
            res.status(502).json({ error: 'Nao foi possivel carregar os leads pendentes.' });
            return;
        }
        const rows = await response.json().catch(() => []);
        if (!Array.isArray(rows)) {
            res.status(502).json({ error: 'Resposta invalida ao carregar os leads pendentes.' });
            return;
        }
        scanned += rows.length;
        rows.forEach((row) => {
            const lead = remarketingLeadForAdmin(row, delayMinutes);
            if (lead && (!kitOnly || isKitRemarketingLead(lead))) leads.push(lead);
        });
        if (rows.length < take) {
            exhausted = true;
            break;
    ...
    const filteredLeads = leads.slice(0, limit)
        .filter((lead) => !query || [lead.name, lead.phone, lead.email, lead.txid]
            .some((value) => String(value || '').toLowerCase().includes(query)));
    filteredLeads.sort((a, b) => {
        if (kitOnly) return (Date.parse(b.createdAt || '') || 0) - (Date.parse(a.createdAt || '') || 0);
        if (a.automaticReady !== b.automaticReady) return a.automaticReady ? -1 : 1;
    ...
    res.status(200).json({
        ...
        summary: {
            ...
            ready: filteredLeads.filter((lead) => lead.automaticReady).length,
            delayMinutes,
            kitOnly,
            scanned,
            truncated: kitOnly && !exhausted && scanned >= maxScanned && leads.length < limit
        },
        variables: REMARKETING_SMS_VARIABLES
    });
}

async function smsMaisRemarketingSend(req, res) {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Method not allowed' });
        return;
    }
    ...
    const sessionIds = [...new Set((Array.isArray(body.sessionIds) ? body.sessionIds : [])
        .map((value) => String(value || '').trim()).filter(Boolean))];
    const kitOnly = body.kitOnly === true;
    if (!sessionIds.length) {
        res.status(400).json({ error: 'Selecione pelo menos um lead pendente.' });
        return;
    }
    if (sessionIds.length > 20) {
        res.status(400).json({ error: 'Selecione ate 20 leads por envio.' });
        return;
    }
    const settingsData = await getSettings().catch(() => ({}));
    const smsConfig = asObject(settingsData?.smsmais);
    if (smsConfig.enabled !== true || !String(smsConfig.token || '').trim()) {
        res.status(400).json({ error: 'Ative a integracao e configure o token da SMSMais antes de enviar.' });
        return;
    }
    const messageTemplate = String(body.message || smsConfig.remarketingMessage || '').replace(/[\r\n]+/g, ' ').trim().slice(0, 320);
    if (!messageTemplate) {
        res.status(400).json({ error: 'Escreva a mensagem do remarketing.' });
        return;
    }
    const baseUrl = resolveRequestBaseUrl(req) || String(process.env.APP_PUBLIC_URL || 'https://ifoodparceiros.vercel.app');
    const requestedAt = new Date().toISOString();
    const jobs = [];
    const ignored = [];
    for (let index = 0; index < sessionIds.length; index += 8) {
        const chunk = sessionIds.slice(index, index + 8);
        const resolved = await Promise.all(chunk.map(async (sessionId) => {
            const found = await getLeadBySessionId(sessionId).catch(() => ({ ok: false, data: null }));
            const lead = found?.ok ? found.data : null;
            const adminLead = lead ? remarketingLeadForAdmin(lead, 10) : null;
            if (!adminLead || (kitOnly && !isKitRemarketingLead(adminLead))) {
                return { ignored: { sessionId, reason: 'lead_paid_or_not_eligible' } };
            }
            return {
                job: {
                    channel: 'smsmais',
                    ...
                    dedupeKey: `smsmais:remarketing:${adminLead.sessionId}:${adminLead.txid}`,
                    scheduledAt: requestedAt,
                    payload: {
                        sessionId: adminLead.sessionId,
                        txid: adminLead.txid,
                        baseUrl,
                        source: 'manual',
                        requestedAt,
        ...
        resolved.forEach((item) => {
            if (item?.job) jobs.push(item.job);
            if (item?.ignored) ignored.push(item.ignored);
        });
    }
    const queueResults = await Promise.all(jobs.map((job) => enqueueManualRemarketingSms(job).catch((error) => ({
        ok: false,
        reason: error?.message || 'queue_error'
    }))));
    const accepted = queueResults.filter((result) => result?.ok && result?.queued).length;
    const alreadyQueued = queueResults.filter((result) => result?.ok && !result?.queued).length;
    const processing = accepted > 0
        ? await processDispatchQueue(Math.min(Math.max(accepted + 20, 40), 300)).catch(() => null)
        : null;
    res.status(200).json({
        ok: true,
        ...
        failedToQueue: queueResults.filter((result) => !result?.ok).length,
        processing
    });
}

async function smsMaisHistory(req, res) {
    if (req.method !== 'GET') {
        res.status(405).json({ error: 'Method not allowed' });
        return;
    }
    ...
    const limit = clamp(req.query?.limit || 100, 10, 300);
    const url = new URL(`${SUPABASE_URL}/rest/v1/event_dispatch_queue`);
    url.searchParams.set('select', 'id,event_name,kind,payload,status,attempts,last_error,scheduled_at,processed_at,created_at');
    url.searchParams.set('channel', 'eq.smsmais');
    url.searchParams.set('kind', `eq.${REMARKETING_SMS_KIND}`);
    url.searchParams.set('order', 'created_at.desc');
    url.searchParams.set('limit', String(limit));
    const response = await fetchFn(url.toString(), { headers: smsMaisSupabaseHeaders() }).catch(() => null);
    if (!response?.ok) {
        res.status(502).json({ error: 'Nao foi possivel carregar o historico de SMS.' });
        return;
    }
    const rows = await response.json().catch(() => []);
    const history = (Array.isArray(rows) ? rows : []).map((row) => {
        ...
        const outcome = String(delivery?.outcome || (row?.status === 'failed' ? 'failed' : row?.status || 'pending'));
        return {
            id: row.id,
            source: String(delivery?.source || payload?.source || 'automatic'),
            sessionId: String(delivery?.sessionId || payload?.sessionId || ''),
            txid: String(delivery?.txid || payload?.txid || ''),
            name: String(delivery?.leadName || ''),
            phone: String(delivery?.phone || ''),
            message: String(delivery?.message || ''),
            amount: Number(delivery?.amount || 0),
            discountedAmount: Number(delivery?.discountedAmount || 0),
            outcome,
            reason: String(delivery?.reason || row?.last_error || ''),
            providerStatus: String(delivery?.providerStatus || ''),
    ...
    res.status(200).json({
        ...
        history,
        summary: {
            sent: history.filter((item) => item.outcome === 'sent').length,
            pending: history.filter((item) => item.outcome === 'pending' || item.outcome === 'processing').length,
            skipped: history.filter((item) => item.outcome === 'skipped').length,
            failed: history.filter((item) => item.outcome === 'failed').length
        }
    });
}

async function fetchSentRemarketingSmsJobs(maxRows = 80000) {
    const rows = [];
    const table = process.env.SUPABASE_DISPATCH_TABLE || 'event_dispatch_queue';
    let offset = 0;
    let truncated = false;
    while (rows.length < maxRows) {
        const take = Math.min(250, maxRows - rows.length);
        const url = new URL(`${SUPABASE_URL}/rest/v1/${table}`);
        url.searchParams.set('select', 'payload,processed_at');
        url.searchParams.set('channel', 'eq.smsmais');
        url.searchParams.set('kind', `eq.${REMARKETING_SMS_KIND}`);
        url.searchParams.set('status', 'eq.done');
        url.searchParams.set('order', 'created_at.asc');
        url.searchParams.set('limit', String(take));
        url.searchParams.set('offset', String(offset));
        const response = await fetchFn(url.toString(), { headers: smsMaisSupabaseHeaders() }).catch(() => null);
        if (!response?.ok) return { ok: false, rows, truncated, status: response?.status || 0 };
        const page = await response.json().catch(() => []);
        if (!Array.isArray(page)) return { ok: false, rows, truncated, status: response.status };
        rows.push(...page);
...
async function smsMaisRemarketingSales(req, res) {
    if (req.method !== 'GET') {
        res.status(405).json({ error: 'Method not allowed' });
        return;
    }
    ...
    const maxRows = clamp(firstQueryValue(req.query?.max) || 80000, 1, MAX_LEADS_EXPORT_ROWS);
    const [result, smsResult] = await Promise.all([
        fetchAllLeadsForExport({
            maxRows,
            pageSize: 250,
            select: REMARKETING_SALES_SELECT_FIELDS,
            enrich: false
        }),
        fetchSentRemarketingSmsJobs(maxRows)
    ]);
    if (!result.ok) {
        res.status(502).json({
            error: 'Nao foi possivel consultar os pagamentos confirmados.',
            source: 'leads',
    ...
    const allSales = [];
    (Array.isArray(result.rows) ? result.rows : []).forEach((row) => {
        allSales.push(...extractGatewaySalesEntries(row));
    });
    const report = buildRemarketingSalesReport(allSales, smsResult.ok ? smsResult.rows : []);
    res.status(200).json({
        ok: true,
        summary: {
            ...report.summary,
            smsAvailable: smsResult.ok,
            scannedRows: Number(result.rows?.length || 0),
            truncated: result.truncated === true || smsResult.truncated === true
        },
        sales: report.sales.slice(0, 150)
    });
}
...
module.exports = async (req, res) => {
    ...
    switch (route) {
        ...
        case 'smsmais-remarketing-leads':
            return smsMaisRemarketingLeads(req, res);
        case 'smsmais-remarketing-send':
            return smsMaisRemarketingSend(req, res);
        case 'smsmais-history':
            return smsMaisHistory(req, res);
        case 'smsmais-remarketing-sales':
            return smsMaisRemarketingSales(req, res);
...
module.exports._internals = { extractGatewaySalesEntries, remarketingLeadForAdmin, isKitRemarketingLead };
