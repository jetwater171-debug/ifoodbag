...
function extractGatewaySalesEntries(row = {}) {
    ...
    const pushEntry = (item = {}, source = 'history') => {
        ...
        entries.push({
            ...
            shippingId: String(item?.shippingId || row?.shipping_id || payload?.shipping?.id || '').trim(),
...
function isRemarketingSale(entry = {}) {
    const markers = [
        entry?.step,
        entry?.shippingId,
        entry?.shippingName,
        ...
        .replace(/[ç]/g, 'c')
        .replace(/[ãáàâ]/g, 'a');
    return markers.includes('remarketing_recovery') || markers.includes('remarketing') || markers.includes('recuperacao');
}
...
async function smsMaisRemarketingSales(req, res) {
    ...
    const sales = [];
    (Array.isArray(result.rows) ? result.rows : []).forEach((row) => {
        extractGatewaySalesEntries(row).forEach((entry) => {
            if (isRemarketingSale(entry)) sales.push(entry);
        });
    });
    ...
    const totalRevenue = sales.reduce((sum, sale) => Number((sum + Number(sale?.amount || 0)).toFixed(2)), 0);
    const uniqueSessions = new Set(sales.map((sale) => String(sale?.sessionId || '').trim()).filter(Boolean));
    res.status(200).json({
        ok: true,
        summary: {
            sales: sales.length,
            revenue: totalRevenue,
            averageTicket: sales.length ? Number((totalRevenue / sales.length).toFixed(2)) : 0,
            customers: uniqueSessions.size,
