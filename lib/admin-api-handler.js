...
async function smsMaisRemarketingLeads(req, res) {
    ...
    const limit = clamp(req.query?.limit || 500, 20, 1000);
    ...
    const leads = (Array.isArray(rows) ? rows : [])
        .map((lead) => remarketingLeadForAdmin(lead, delayMinutes))
        .filter(Boolean)
    ...
    res.status(200).json({
        ...
        summary: {
            ...
            ready: leads.filter((lead) => lead.automaticReady).length,
            delayMinutes
        },
...
async function smsMaisRemarketingSend(req, res) {
    ...
    for (let index = 0; index < sessionIds.length; index += 8) {
        ...
        const resolved = await Promise.all(chunk.map(async (sessionId) => {
            ...
            if (!adminLead) return { ignored: { sessionId, reason: 'lead_paid_or_not_eligible' } };
            return {
                job: {
                    channel: 'smsmais',
...
module.exports._internals = { extractGatewaySalesEntries };
