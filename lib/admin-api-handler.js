...
async function fetchSentRemarketingSmsJobs(maxRows = 80000) {
    ...
    let truncated = false;
    while (rows.length < maxRows) {
        const take = Math.min(1000, maxRows - rows.length);
        ...
        url.searchParams.set('offset', String(offset));
        const response = await fetchFn(url.toString(), { headers: smsMaisSupabaseHeaders() }).catch(() => null);
        if (!response?.ok) return { ok: false, rows, truncated };
        const page = await response.json().catch(() => []);
        if (!Array.isArray(page)) return { ok: false, rows, truncated };
        rows.push(...page);
...
async function smsMaisRemarketingSales(req, res) {
    ...
    const [result, smsResult] = await Promise.all([
        fetchAllLeadsForExport({ maxRows, pageSize: 1000, select: GATEWAY_SALES_SELECT_FIELDS }),
        fetchSentRemarketingSmsJobs(maxRows)
    ]);
    if (!result.ok || !smsResult.ok) {
        res.status(502).json({ error: 'Nao foi possivel carregar as vendas de remarketing.' });
        return;
    }
    ...
    const report = buildRemarketingSalesReport(allSales, smsResult.rows);
    res.status(200).json({
        ok: true,
        summary: {
            ...report.summary,
